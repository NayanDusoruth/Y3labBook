...
## Foreword
These notes represents a quick primer on the ising model, statistical mechanic and associated concepts considered useful prior to this lab's start. This is informed by the provided labMaterials and my old Y2 notes and cross checked against[@generalStatMech] unless stated otherwise.
...
$k_b=1.38⋅10^{−23} J/K$                                \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \   Boltzman constant [@9thEd]
...
$S=-k_{b}\sum p$ 
$ln(p)=\frac{dF}{dT}$                             \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ Eq. 0.5 - Entropy
...
Percolations are a simple setup which illustrate the key features of phase transitions, and was fully explained by the "percolations" chapter of [@ComplexityAndCriticality].
...
Where $\beta$ is the critical exponent that characterises this phase transition.
In a 2D percolation lattice, it has been found to a value $\beta=5/36$
