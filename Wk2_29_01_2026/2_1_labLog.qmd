# Wk 2 Lab log
## Wk 2 summary
This week involved myself reading up on more of the theory, building up the dataset object, and debugging some of the basic observables. Lab partner began writing code for cluster analysis.

## 28/01/2026 note:
Have begun switching from onenote labbook to a quarto document.

## Lab log 29/01/2026
Dr Turci session; went over Jaynes model and mean field theory derivion

catchup w/lab partner; outlined code
She summarised lit review:

* cluster definition:
	* useful definition; connected points w/same spin; [@ClusterDombAndStolS] 
...
This was done to allow for parallel development of code
...
observables will be average of values over simulation steps.
implemented partition function, entropy, helmholtz free energy, as observables.
implemented "average" observable methods - they get the average of a given observable between simulation step indices
...
The critical exponent theory follows heavily from [@CritExpLink3] which seems to handle scaling phenomena.
As it stands, there may be two approaches to computing the critical exponents:

* Simple case; vary models over temperature and fit some variation of $(T-T_{c})^{x}$ where x is a critical parameter to various observables (as seen in [@ComplexityAndCriticality] P132-134)
* Attempt to follow the procedure done by [@CritExpLink3]
...
![Fig 2.3 - Config 0](figures/magTestConfig0.png){fig-align="left"}

![Fig 2.4 - Config 15](figures/magTestConfig15.png){fig-align="left"}

![Fig 2.5 - Config 16](figures/magTestConfig16.png){fig-align="left"}

![Fig 2.6 - Config 19](figures/magTestConfig19.png){fig-align="left"}
...
* magnetisation computation is going wrong somehwhere; as highly ordered states (I.E fig 2.3) are reading low magnetisation whilst disorded states (I.E fig 2.5) are reading high magnetisation. This contradicts an understanding of the underlying theory that ordered ising models should be more magnetised
...
May need to modify dataset object to allow different configs to have different equilibration times; [@CritExpLink3] suggests that equilibration time may vary over parameters

Furthermore may need to implement system to estimate equilibration times:

* dual system introduced by [@CritExpLink3] - create 2 configs; one starting all spin up, one all spin down; compare magnetisations of two results until they are within error of each other
