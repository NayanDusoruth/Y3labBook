...
###05/03/2026
Fiddling with analytical specifics (mostly the time block and RMS mag mean radius). Refining the time block improves the $T_{c}$ estimate; which has a knock on effect on the $\beta$ estimate. have also streamlined the data generation-> latex report doc pipeline; it now autofills the final results into the report. it writes the results as latex commands of form \newcommand{\value}{\value \pm dev} to a seperate .tex doc which is imported by the report file. That way results can be quited as \value, and it will autoupdate every time the main analytical code is run.
...
$n_{connections d=3}=(l-3)^{3}\cdot6+8\cdot(l-2)^{2}\cdot5+8(l-2)\cdot2+8\cdot3$  Eq. 7.2
...
Not going to include it in the report; its interesting but doesn't fit well and i'm unsure how to address these points...


### 08/03/2026
Note to self; may be worth showing model exemplars below, above and at critical temperature.
